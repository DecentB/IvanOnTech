...
contract('CoinFlip', async function(accounts) {
  ...
  it('should pay out if win and not if lose', async function(){
    instance = await coinFlip.new();
    await instance.addFunds({value: web3.utils.toWei('10', 'ether')});
    let playerBlockchainBalanceBefore = parseFloat(await web3.eth.getBalance(accounts[1]));

    let result = await instance.placeBet(0, {from: accounts[1], value: web3.utils.toWei('1.5', 'ether')});

    if (result.logs[0].event == 'win') {
      console.log('win')
      let blockchainBalance = parseFloat(await web3.eth.getBalance(instance.address));
      let playerBlockchainBalanceAfter = await parseFloat(await web3.eth.getBalance(accounts[1]));
      assert (blockchainBalance ==  web3.utils.toWei('8.5', 'ether')) &&
        playerBlockchainBalanceAfter == (playerBlockchainBalanceBefore + web3.utils.toWei('1.5', 'ether'));  
    } else if (result.logs[0].event == 'lose'){
      console.log('lose')
      let blockchainBalance = parseFloat(await web3.eth.getBalance(instance.address));
      let playerBlockchainBalanceAfter = await parseFloat(await web3.eth.getBalance(accounts[1]));
      assert (blockchainBalance ==  web3.utils.toWei('11.5', 'ether')) &&
        playerBlockchainBalanceAfter == (playerBlockchainBalanceBefore - web3.utils.toWei('1.5', 'ether')); 
    } else {
